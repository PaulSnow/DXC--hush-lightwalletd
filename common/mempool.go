...
var (
	...
	// The hushd RPC `getrawmempool` returns the entire mempool each time, so
	...
	// The most recent hushd getblockchaininfo reply, for height and best block
	// hash (tip) which is used to detect when a new block arrives.
	g_lastBlockChainInfo *HushdRpcReplyGetblockchaininfo = &HushdRpcReplyGetblockchaininfo{}
